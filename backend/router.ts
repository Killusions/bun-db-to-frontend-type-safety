...
const baseRouter = router({
  getUserInfo: publicProcedure.input(createSelectSchema(tables.users).pick({ id: true })).query(async ({ input }) => {
    ...
    return createInsertSchema(tables.users).omit({ email: true, createdAt: true, hashedPassword: true }).parse(user);
  }),
  getUserByEmail: adminProcedure.input(createSelectSchema(tables.users).pick({ email: true })).query(async ({ input }) => {
    ...
    return createSelectSchema(tables.users).omit({ hashedPassword: true }).extend({ userRoles: createSelectSchema(tables.userRoles).array() }).parse(user);
  }),
});
