...
export const authRouter = router({
  register: publicProcedure.input(createSchema.omit({ role: true })).mutation(async ({ input }) => {
    ...
    return createInsertSchema(tables.users).pick({ id: true }).parse({ id: account.id });
  }),
  login: publicProcedure.input(userActionSchema.omit({ name: true })).mutation(async ({ input, ctx }) => {
    ...
    return userActionSchema.pick({ redirectTo: true }).required({ redirectTo: true }).parse({ redirectTo: input.redirectTo ?? '/' });
  }),
  ...
  createAccount: adminProcedure.input(createSchema).mutation(async ({ input }) => {
    const account = await createAccount(input);
    return createSelectSchema(tables.users).pick({ id: true }).parse({ id: account.id });
  })
});
