...
async function installNativeDependencies(options: PrebuildOptions) {
  ...
  if ((options.platform === 'android' || options.platform === 'all') && existsSync(ANDROID_DIR)) {
  ...
  if ((options.platform === 'ios' || options.platform === 'all') && existsSync(IOS_DIR)) {
    ...
    if (podResult.exitCode !== 0) {
      console.warn('⚠️  CocoaPods installation failed. You may need to run "pod install" manually.');
    } else {
