...
async function buildAndroid(mode: 'development' | 'release', clean: boolean = false) {
  ...
  // Build the APK
  const buildCommand = mode === 'release' 
  ...
  if (existsSync(apkPath)) {
    ...
    if (copyResult.exitCode === 0) {
      console.log(`✅ Android ${mode} APK built successfully: ${targetApkPath}`);
    } else {
...
async function buildIOS(mode: 'development' | 'release', clean: boolean = false) {
  ...
  const archiveResult = spawnSync({
    cmd: [
      'xcodebuild',
      'archive',
      '-workspace', 'SimpleServerMobile.xcworkspace',
      '-scheme', 'SimpleServerMobile',
...
async function buildWeb(mode: 'development' | 'release') {
  ...
  // Create tarball
  const tarballPath = join(BUILD_DIR, `simple-server-mobile-web-${mode}.tar.gz`);
...
async function runBuild(options: BuildOptions) {
  console.log(`🚀 Starting ${options.platform} build in ${options.mode} mode...`);
  ...
  try {
    ...
    cmd: ['ls', '-la', BUILD_DIR], stdio: ['ignore', 'pipe', 'inherit'] });
...
const platform = args.find(arg => ['android', 'ios', 'web', 'all'].includes(arg)) as BuildOptions['platform'] || 'all';
